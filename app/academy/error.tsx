...
export default function AcademyError({
  ...
  useEffect(() => {
    console.error('[Academy Error Boundary]', error?.message ?? error);
  }, [error]);
  ...
  return (
    ...
    >
      ...
      <h2 className="text-lg font-semibold text-slate-100 mb-2">מרכז הלמידה אינו זמין</h2>
      <p className="text-sm text-slate-400 max-w-md mb-6">
        לא ניתן לטעון את מרכז הלמידה. ייתכן שהחיבור לבסיס הוקטורי (Pinecone) כשל.
      </p>
      {error?.message ? (
