...
export async function GET(request: NextRequest) {
  ...
  if (isSessionEnabled()) {
    ...
    if (!session || !hasRequiredRole(session.role, 'admin')) {
      if (typeof console !== 'undefined' && console.warn) {
        console.warn('[API ops/metrics] 401 Unauthorized: missing or invalid session or not admin.');
      }
  ...
  } = { fromHistorical: false, winRatePct: null, avgErrorPct: null, last10WinRatePct: null, prev10WinRatePct: null };

  try {
    const historical = await listHistoricalPredictions(20);
    if (historical.length > 0) {
      const hits = historical.filter((r) => HIT_LABELS.includes(r.outcome_label)).length;
      accuracy = {
        fromHistorical: true,
        winRatePct: Math.round((hits / historical.length) * 1000) / 10,
        avgErrorPct: Math.round((historical.reduce((s, r) => s + r.absolute_error_pct, 0) / historical.length) * 100) / 100,
        last10WinRatePct: historical.length >= 10
          ? Math.round((historical.slice(0, 10).filter((r) => HIT_LABELS.includes(r.outcome_label)).length / 10) * 1000) / 10
          : null,
        prev10WinRatePct: historical.length >= 20
          ? Math.round((historical.slice(10, 20).filter((r) => HIT_LABELS.includes(r.outcome_label)).length / 10) * 1000) / 10
          : null,
      };
    }
  } catch {
