...
export default function BacktestError({
  ...
  useEffect(() => {
    console.error('[Backtest Error Boundary]', error?.message ?? error, error?.digest ? `digest=${error.digest}` : '');
  }, [error]);
  ...
  return (
    <div className="min-h-[280px] flex flex-col items-center justify-center p-6 bg-slate-900/95 border border-slate-700/80 rounded-xl text-center" dir="rtl">
      ...
      <h2 className="text-lg font-semibold text-slate-100 mb-2">שגיאה ב-Backtest</h2>
      <p className={`text-sm text-slate-400 max-w-md ${msg ? 'mb-2' : 'mb-6'}`}>
        אירעה תקלה בטעינת מנוע ה-Backtest. ניתן לנסות שוב.
      </p>
      {msg ? (
