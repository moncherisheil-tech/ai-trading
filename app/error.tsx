...
export default function Error({
  ...
  useEffect(() => {
    console.error('[Error Boundary]', error?.message ?? error, error?.digest ? `digest=${error.digest}` : '');
  }, [error]);

  const technicalMessage = error?.message || (typeof error === 'string' ? error : 'Unknown error');
  ...
  return (
    <div className="min-h-[280px] flex flex-col items-center justify-center p-6 bg-slate-900/95 border border-slate-700/80 rounded-xl text-center" dir="rtl">
      ...
      <p className={`text-sm text-slate-400 max-w-md ${technicalMessage ? 'mb-2' : 'mb-6'}`}>
