...
export async function listVirtualTradeHistory(limit = 50): Promise<VirtualTradeHistoryRow[]> {
  ...
  try {
    const { rows } = await sql`
      SELECT
        id, event_id, prediction_id, symbol, signal_side,
        confidence::float AS confidence, mode, executed, execution_status, reason,
        overseer_summary, overseer_reasoning_path,
        ...
        virtual_trade_id, created_at::text AS created_at
      FROM virtual_trades_history
      ORDER BY created_at DESC
      LIMIT ${limit}
    `;
    return (rows || []).map(mapHistoryRow) as VirtualTradeHistoryRow[];
  } catch (err) {
...
function mapHistoryRow(r: Record<string, unknown>): VirtualTradeHistoryRow {
  return {
    id: Number(r.id),
