...
/**
 * Computes P_success from the dynamic formula using weights in system_configs (Vercel Postgres).
 * P_success = ((W_vol × VolΔ) + (W_rsi × RSI) + (W_sent × Sent)) / RiskFactor
 * Returns value in 0–100 range (capped).
 */
export async function computePSuccess(inputs: FormulaInputs, weights?: { volume: number; rsi: number; sentiment: number }): Promise<number> {
...
/**
 * Generates a maximum of 2 short, highly decisive sentences in simple Hebrew ("השורה התחתונה").
 * Kept brief for beginners; no long explanations.
 */
export function buildHebrewReport(params: {
  ...
  const base = `${symbol}: צפי ${dirHe} בהסתברות ${probability}%. ${riskLevelHe}.`;
  const actionHint =
    direction === 'Bullish' && probability >= 60
      ? ' מומלץ לשקול כניסה בהתאם לרמת הסיכון.'
      : direction === 'Bearish' && probability >= 60
        ? ' מומלץ להיזהר או לצמצם חשיפה.'
        : direction === 'Neutral' || probability < 55
          ? ' מומלץ להמתין לסימנים ברורים.'
          : ' מומלץ להמתין לירידה קלה לפני כניסה.';
  const full = `השורה התחתונה: ${base}${actionHint}`;
  const bottom_line_he = full.length > 120 ? full.slice(0, 117) + '…' : full;
