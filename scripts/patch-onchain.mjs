...
if (start < 0) throw new Error('onchain start');
const ret = "  return { onchain_score: Math.max(0, Math.min(100, Number(out.onchain_score) ?? 50)), onchain_logic: String(out.onchain_logic || 'ללא נימוק').slice(0, 500) };";
const end = s.indexOf(ret, start);
if (end < 0) throw new Error('onchain end');
const end2 = end + ret.length;
const repl = String.raw`  let anthropicKey: string | undefined;
  try {
...
console.log('onchain anthropic ok');
