...
const ret = "  const onchain_score = Math.max(0, Math.min(100, Number(out.onchain_score) ?? 50));\r\n  return { onchain_score, onchain_logic: String(out.onchain_logic || 'ללא נימוק').slice(0, 500) };";
const end = s.indexOf(ret, start);
if (start < 0 || end < 0) throw new Error('bad');
...
const repl = `  let anthropicKey: string | undefined;
  ...
  return { onchain_score, onchain_logic: String(out.onchain_logic || 'ללא נימוק').slice(0, 500) };`;
s = s.slice(0, start) + repl.replace(/\n/g, '\r\n') + s.slice(end2);
fs.writeFileSync(p, s);
console.log('onchain patched');
