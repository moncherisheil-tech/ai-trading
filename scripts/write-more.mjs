...
const depth = `import type { BinanceDepthSnapshot } from '@/lib/api-utils';
...
`;
fs.mkdirSync(path.join(root, "lib/market"), { recursive: true });
fs.writeFileSync(path.join(root, "lib/market/binance-depth-hints.ts"), depth, "utf8");
const idx = `export { fetchS1MacroSatellite, type S1MacroSnapshot } from '@/lib/satellites/s1-macro';
export { buildS2MicroSatelliteSummary, type S2MicroInput } from '@/lib/satellites/s2-microstructure';
`;
fs.writeFileSync(path.join(root, "lib/satellites/index.ts"), idx, "utf8");
